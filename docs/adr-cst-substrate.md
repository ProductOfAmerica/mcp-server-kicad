...
- 2026-08-09: slice 12A: hardened CST repr (whitespace folded into per-node sep slots, per-kind node classes with class-attr constants, parse-time token interning). Measured on real boards against the 37.4x baseline: 11.35x retained on the 6MB Video board, 8.85x on the 71MB vme-wren. The <=10x Video gate MISSES by 13%: board-tool conversion stays blocked; next decision is flat-token-array redesign vs a consciously revised gate. Gate test lives as a documented xfail.
