...
- `windows.yml`: Windows with KiCad 9, same trigger as the macOS job and the same reason. It installs KiCad because the `_KICAD_WIN_DIRS` probe is otherwise only tested against synthetic directory trees. Windows-specific faults have a history of reaching users first, so a PR being green says nothing about this platform.
