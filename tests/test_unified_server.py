...
class TestUnifiedServer:
    ...
    def test_copy_tools_with_cli(self):
        ...
        # Total tool count
        assert len(registered) == 71, f"Expected 71 tools, got {len(registered)}: {registered}"
    ...
    def test_copy_tools_without_cli(self):
        ...
        # Tool count: 71 total - 17 CLI = 54
        assert len(registered) == 54, f"Expected 54 non-CLI tools, got {len(registered)}"
