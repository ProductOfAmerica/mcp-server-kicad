...
class TestUnifiedServer:
    ...
    def test_copy_tools_with_cli(self):
        ...
        # Total tool count
        assert len(registered) == 72, f"Expected 72 tools, got {len(registered)}: {registered}"
    ...
    def test_copy_tools_without_cli(self):
        ...
        # Tool count: 72 total - 17 CLI = 55
        assert len(registered) == 55, f"Expected 55 non-CLI tools, got {len(registered)}"
