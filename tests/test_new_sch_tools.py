...
from conftest import reparse, assert_erc_clean, build_r_symbol, place_r1, new_schematic
...
class TestMoveComponent:
    def test_move_existing(self, scratch_sch):
        ...
        r1 = next(s for s in sch.schematicSymbols
...
class TestEditComponentValue:
    ...
    def test_edit_reference(self, scratch_sch):
        ...
        sch = reparse(str(scratch_sch))
        refs = [next((p.value for p in s.properties if p.key == "Reference"), None)
...
class TestAddGlobalLabel:
    ...
    def test_with_shape(self, scratch_sch):
        schematic.add_global_label("SDA", 60, 60, shape="bidirectional",
...
class TestAddPowerSymbol:
    def test_basic(self, scratch_sch):
        result = schematic.add_power_symbol("power:VCC", "VCC1", 100, 80,
                                            schematic_path=str(scratch_sch))
